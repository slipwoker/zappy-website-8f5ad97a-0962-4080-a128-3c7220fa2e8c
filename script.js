document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        function clearError(input) {
            const fieldGroup = input.closest('.form-group') || input.parentElement;
            const errorEl = fieldGroup.querySelector('.form-error');
        ...
        function validateField(input) {
            ...
            if (value) {
                ...
                if (input.hasAttribute('minlength') && value.length < parseInt(input.getAttribute('minlength'))) {
                    ...
                    showError(input, isRTL ? `يجب أن يكون ${min} أحرف على الأقل` : `Must be at least ${min} characters`);
                    return false;
                }
                
                if (input.hasAttribute('maxlength') && value.length > parseInt(input.getAttribute('maxlength'))) {
                    const max = input.getAttribute('maxlength');
                    showError(input, isRTL ? `يجب ألا يتجاوز ${max} حرفاً` : `Must not exceed ${max} characters`);
                    return false;
                }
            }
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            if (isValid) {
                ...
                .then(data => {
                    ...
                    successMsg.textContent = isRTL ? 'تم إرسال رسالتك بنجاح!' : 'Your message has been sent successfully!';
                ...
                .catch(error => {
                    ...
                    errorMsg.style.cssText = 'background: #fc8181; color: white; padding: 1rem; border-radius: 0.375rem; margin-top: 1rem; text-align: center;';
                    errorMsg.textContent = isRTL ? 'حدث خطأ. يرجى المحاولة مرة أخرى.' : 'An error occurred. Please try again.';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '8f5ad97a-0962-4080-a128-3c7220fa2e8c',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '8f5ad97a-0962-4080-a128-3c7220fa2e8c';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '8f5ad97a-0962-4080-a128-3c7220fa2e8c';
